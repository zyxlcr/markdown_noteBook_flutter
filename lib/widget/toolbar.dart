...
class MarkdownToolbar extends StatefulWidget {
  ...
  /// Set a custom tooltip [String] for the save button. Leave blank `''` to disable tooltip.
...
class MarkdownToolbarState extends State<MarkdownToolbar> {
    ...
    if (collapsable && isCollapsed) {
      ...
      return [
        ...
        if (!hideSave)
          _buildToolbarItem(
            icon: Icons.save,
            tooltip: showTooltips == true ? saveTooltip : '',
            onPressed: () {
              print("onPressed");
  ...
  void onSavePressed() =>
      onToolbarItemPressed(markdownToolbarOption: MarkdownToolbarOption.save);
